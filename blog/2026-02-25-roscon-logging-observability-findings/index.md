...
We then looked at the 3 talks we thought would be most relevant. The talks are as follows:
...
This talk sounded very similar talk already given directly to this group (see the [meeting in question](/meetings/#2025-06-16-guest-expert-benji-barash-from-roboto-ai)). We decided to skip reviewing this talk in-depth due to the similarity to the previous talk.
...
The group found the talk so relevant that we invited Guillaume to give a more in-depth, technical talk directly to us in January 2026. The talk is available [here](/meetings/#2026-01-28-guest-expert-mirko-from-canonical).
...
We made some observations based on the talks and how they fit with our ongoing.

1. **Make versus Buy decision**: Broadly, tools seem to break down into:
    1. End-to-end service: provides a set of tools to a paying customer, such as log analysis and AI insights. Roboto AI is an example of this.
    2. Developer tooling: provides a way for developers to launch their own observability tools, such as Foxglove Studio, which attach to their robots. The Canonical Observability Stack is an example of this.
  
This fits with our observations in the past, and leads to customers either paying for a pre-built service or choosing to build their own with increasingly better tools available.
...
The group aims to produce a community guide on Logging & Observability. Based on advice from these talks, we are likely to include the following:

- Practical patterns and habits
    - Developers are familiar with the concept of adding a test when fixing a bug to prevent the bug occurring again. We will recommend also creating a diagnostic that can be reported by the robot and give alerts using observability tools.
- Design considerations for a Logging & Observability system, such as:
    - Treating a robot as a client rather than a server
...
- The distinction between vendor-provided tools and building your own stack; in short, the make vs buy trade-offs
